  ...
  return (
    <article className="bg-white h-full rounded-[5px] shadow border border-black border-opacity-10 flex flex-col items-stretch justify-between space-y-2">
      <div className="p-0 relative">
        <Link href="#">
        {cover && <Image src={cover} alt={title} placeholder="blur" className="h-[15rem]" />}
        </Link>

        <div className="absolute bottom-0 left-0 px-4 py-2 text-white text-sm hover:bg-white hover:text-blue-900 transition duration-500 ease-in-out gap-2">
            {tags?.map((tag) => (
              <Tag tag={tag} key={tag} />
            ))}
          </div>
      ...
      <div className="py-6 px-4 space-y-2">
        ...
        <div className="block font-sans text-base font-light leading-relaxed text-inherit antialiased overflow-hidden whitespace-nowrap overflow-ellipsis">{description}</div>
